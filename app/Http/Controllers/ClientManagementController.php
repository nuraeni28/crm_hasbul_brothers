...
class ClientManagementController extends Controller
{
    ...
    {
        Log::info('dataEdit called with input:', $request->all());
        ...
        try {
            ...
            // Logging success
            Log::info('Client data updated successfully for client ID: ' . $clientId);
            // Send success response
            return response()->json(['message' => 'success'], 200);
        } catch (\Exception $e) {
            ...
            // Logging error
            Log::error('Failed to update client data', ['error' => $e->getMessage()]);

            return response()->json(['error' => 'Failed to update client data', 'details' => $e->getMessage()], 500);
        }
    }
    ...
    public function dashboardView(Request $request)
    {
        ...
        $clientRecords = ClientRcdSales::where('client_main_id', $clientId)->whereYear('record_date', $currentYear)->get();
        ...
        $clientLatestReserve = ClientRcdSales::where('client_main_id', $clientId)->orderBy('id', 'asc')->first();
        ...
        if ($clientPayment && $clientPayment->client_payment == 0) {
            $depositRecord = ClientPaymentBalance::where('client_main_id', $clientId)->orderBy('payment_iteration', 'asc')->get();
        }
    ...
    public function salesView(Request $request)
    {
        ...
        $clientSalesRecords = ClientRcdSales::where('client_main_id', $clientId)->orderBy('record_date', 'ASC')->get();
        $result = [];
    ...
    public function salesAdd(Request $request): JsonResponse
    {
        ...
        $clientRecordYears = DB::table('client_rcd_sales')->select('id')->where('client_main_id', $clientId)->whereYear('record_date', $currentYear)->first();
        ...
        if (!$clientRecordYears) {
            ...
            $clientLastYear = DB::table('client_rcd_sales')->select(DB::raw('YEAR(record_date) as year'))->where('client_main_id', $clientId)->groupBy(DB::raw('YEAR(record_date)'))->orderBy(DB::raw('YEAR(record_date)'), 'asc')->first();
    ...
    public function salesEdit(Request $request)
    {
        ...
        foreach ($data as $client_record_month) {
            ClientRcdSales::where('id', $client_record_month[0]['table_id'])->update([
    ...
    public function salesDelete(Request $request): JsonResponse
    {
        ...
        DB::table('client_rcd_sales')->where('client_main_id', $clientId)->whereYear('record_date', $year)->delete();
    ...
    public function btmView(Request $request): JsonResponse
    {
        ...
        if ($clientBtmRecords) {
            $plotRadar = [$clientBtmRecords->btm_hr, $clientBtmRecords->btm_leader_culture, $clientBtmRecords->btm_marketing_branding, $clientBtmRecords->btm_dig_marketing, $clientBtmRecords->btm_sales_cust, $clientBtmRecords->btm_rnd, $clientBtmRecords->btm_ops_proces, $clientBtmRecords->btm_acc_finance];
            $dataBtm = [
    ...
    public function btmEdit(Request $request): JsonResponse
    {
        ...
        if ($clientBtmRecords) {
            ...
            DB::table('client_rcd_btm')->where('client_main_id', $clientId)->update($updateData);
        } else {
